...
module.exports = function(emailOptions, sendgridOptions, errCallback) {
  ...
  return {
    emailMe: function(err, req, res, next) {
      sendgrid.send({
        ...
        html: "<h2>" + req.headers.host + "</h2><h3>Error:</h3><pre>" + err + "</pre><h3>Stack:</h3><pre>" + (errToHTML(err)) + "</pre><h3>Request:</h3><pre>" + (errToHTML({
          stack: objToString(req)
        })) + "</pre>"
      }, function(success, message) {
        if (!success) {
          return errCallback(message);
        }
      });
      return next(err);
    }
